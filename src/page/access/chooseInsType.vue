...
export default {
    ...
    methods: {
        ...
        next(){
          // Util.removeSessionData("hasUploadPics")
          // this.$router.push({name:'result'});
            ...
            if(this.queryData.oid){
                ...
                var picInfo1 = [];
                if(GConfig.isInApp){
                  picInfo1 = Util.getLocalData("hasUploadPics") || [];
                } else {
                  picInfo1 = Util.getSessionData("hasUploadPics") || [];
                }
            ...
            var params = {
                ...
                tax:this.isCompulsory?1:0,
                requirement: this.requirement,
                oid: this.queryData.oid||'',
                isTransferCar:this.isTransferCar?1:0
            };
            console.log(params)
            // return
            quoteService.quote(params).then((res) => {
                Toast.clear();
                if(res.code == 1){
                    Util.removeSessionData("hasUploadPics")
    ...
    created() {
        ...
        quoteService.orderProduct({cityCode:this.queryData.cityCode|| ""})
            .then((res) => {
                Toast.clear();
                this.parseData(res)
        })
